...
当前工作区既有的 11 份资料、1,324 条证据、10 份文档评测、冲突样本和图谱用例，统一视为 `development_fixture`：它们用于证明解析、定位、状态恢复和评测代码没有退化，不代表资料已经获得企业业务批准。

真实企业质量闭环从“全库资料地图”开始：

1. 本地只读预读明确指定的资料根目录，先生成文件夹地图和每份文件的中文说明卡。
2. 资料负责人确认哪些文件属于业务范围、文件用途以及哪个版本具有权威性。
3. 系统只对已确认资料进行知识编译，并自动校验原文、定位、哈希和产物一致性。
4. 人工只复核跨文件真冲突、重要 Wiki 结论、例外规则和发布决定。

普通原子证据的 `verified` 兼容状态只能解释为“技术校验通过”，不能作为业务批准、时效有效或权威性的证明。不得再以“把全部证据勾选一遍”作为质量门槛。

`quality status` v3 因此把开发回归基线标为非企业业务基线，并新增真实业务资料范围门槛：至少存在一份 `production + in_scope` 资料、至少一份被确认具有权威性，且不能留有尚未判断范围的生产资料。冲突仍采用分层抽样并主动补充已知真冲突，不要求全量人工标注。
...
内置样例只验证评测框架本身，不能代表企业资料质量。现有10份已批准资料是第一版黄金硬基线；20份仅是可配置覆盖建议，只有出现格式、业务类型或风险缺口时才扩充，不从 NAS 机械凑数。

截至 2026-07-27，质量闭环 v2 的10项硬门槛已全部通过：100条分层冲突样本完成标注和独立复核，7条真实 `value_change` 冲突完成独立复核，图谱黄金集6/6用例通过。以下项目属于后续增强，不再阻塞当前核心产品验收：

- 结论级引用正确率；
- 重要事实遗漏率；
- 按预测类型和相似度区间抽取的跨文档冲突样本，以及人工主动加入的已知真冲突；
- 已知真冲突可通过聚焦批次从尚未标注的高相似度 `value_change` 或 `polarity_change` 候选中主动筛选，不再要求为寻找正例审核普通大批次；
- 时间范围和适用范围准确率；
- 模型辅助模式与证据保真模式的差异对比。
