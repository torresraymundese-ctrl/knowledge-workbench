...
| `web_service.py` | Web 只读投影；复用当前版本/当前处理运行与密级边界 |
| `webapp.py` | 仅绑定本机回环地址的 HTTP/API 服务，不包含业务状态写入 |
...
- 首个 Web 里程碑只开放 `GET`/`HEAD`，拒绝写请求；API 不返回来源路径、原始副本路径、证据原文或审计详情，`restricted` 名称和定位按密级隐藏。
- Web 服务只允许监听 `127.0.0.1` 或 `localhost`，静态页面不调用外部 CDN，并设置 CSP、禁止嵌入、禁止缓存和禁止引用来源泄露等响应头。
...
- OCR、NAS 增量同步、权限主体、审批流和 Web 写操作；
